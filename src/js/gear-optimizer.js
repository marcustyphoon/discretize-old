...
(function ($) {
  ...
  async function run() {
    ...
    input.modifiers.push({
      flat: { 'Agony Resistance': Number.parseInt($(Selector.INPUT.AGONY_RESISTANCE).val(), 10) || 0 }
    });
  ...
  function updateDOM(newList) {
    ...
    for (const [index, element] of newList.entries()) {
      ...
      if (element !== currentList[index]) {
        newItem = characterToRow(element);
        currentList.splice(index, 0, element);
  ...
  function toModal(_character) {
    ...
    $.each(
      ...
      (index, value) => {
        const testCharacter = optimizerCore.clone(_character);
        testCharacter.baseAttributes[value] += 5;
        optimizerCore.updateAttributes(testCharacter);
        effectiveValues[value] = Number(
          (testCharacter.attributes['Damage'] - attributes['Damage']).toFixed(5)
        ).toLocaleString('en-US');
      }
    );
    ...
    $.each(
      ...
      (index, value) => {
        const testCharacter = optimizerCore.clone(_character);
        testCharacter.baseAttributes[value] = Math.max(testCharacter.baseAttributes[value] - 5, 0);
        optimizerCore.updateAttributes(testCharacter);
